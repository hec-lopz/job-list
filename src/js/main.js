...
const renderCards = (async () => {
  ...
  $clear_filters_btn.addEventListener("click", () => {
    $filters_container
      .querySelectorAll(".job-tags__item--active-filter")
      .forEach((el) => {
  ...
  return function (tags = []) {
    tags.forEach((tag) => {
      if (filtered_list.includes(tag)) {
        const cards_to_unhide = $cards.filter(
          (card) => !card.dataset.tags.split(",").includes(tag)
        );
        cards_to_unhide.forEach((card) => {
          card.classList.remove("card--hidden");
        });
      } else {
        const cards_to_hide = $cards.filter(
          (card) => !card.dataset.tags.split(",").includes(tag)
        );
...
const toggleActiveStates = (element, activate = false) => {
  ...
  if (activate === true) {
    const card_tags_to_activate = card_tags.filter(
      (tag) =>
        tag.dataset.value.toLowerCase() === element.dataset.value.toLowerCase()
    );
    card_tags_to_activate.forEach((item) =>
      item.classList.add("job-tags__item--active")
    );
  } else {
    const card_tags_to_deactivate = card_tags.filter(
      (tag) =>
        element.dataset.value.toLowerCase() ===
          tag.dataset.value.toLowerCase() &&
        tag.classList.contains("job-tags__item--active")
    );
    card_tags_to_deactivate.forEach((item) =>
      item.classList.remove("job-tags__item--active")
    );
  }
};
